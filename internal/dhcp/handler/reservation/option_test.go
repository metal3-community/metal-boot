...
import (
	...
	dhcpotel "github.com/metal3-community/metal-boot/internal/otel"
			...
			args: args{
				...
				m: &dhcpv4.DHCPv4{
					Options: dhcpv4.OptionsFromList(
						dhcpv4.OptParameterRequestList(dhcpv4.OptionSubnetMask),
					),
				},
				d: &data.DHCP{
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tt.otelEnabled {
				...
				ctx = dhcpotel.ContextWithTraceparentString(
					ctx,
					"00-23b1e307bb35484f535a1f772c06910e-d887dc3912240434-01",
				)
			}
			bootfile, nextServer := tt.server.bootfileAndNextServer(
				ctx,
				tt.args.pkt,
				tt.args.uClass,
				tt.args.tftp,
...
func TestSetNetworkBootOpts(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"netboot not allowed": {
			...
			want: &dhcpv4.DHCPv4{
				ServerIPAddr: net.IPv4(0, 0, 0, 0),
				BootFileName: "/netboot-not-allowed",
			},
		},
		"netboot allowed": {
			server: &Handler{
				Log: logr.Discard(),
				Netboot: Netboot{IPXEScriptURL: func(*dhcpv4.DHCPv4) *url.URL {
					return &url.URL{
						Scheme: "http",
						Host:   "localhost:8181",
