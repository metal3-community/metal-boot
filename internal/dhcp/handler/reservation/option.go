...
func (h *Handler) setDHCPOpts(
	...
	if d.BroadcastAddress.Compare(netip.Addr{}) != 0 {
		mods = append(
			mods,
			dhcpv4.WithGeneric(dhcpv4.OptionBroadcastAddress, d.BroadcastAddress.AsSlice()),
		)
	}
	if d.DomainName != "" {
	...
	if h.SyslogAddr.Compare(netip.Addr{}) != 0 {
		mods = append(
			mods,
			dhcpv4.WithOption(dhcpv4.OptGeneric(dhcpv4.OptionLogServer, h.SyslogAddr.AsSlice())),
		)
	}
...
func (h *Handler) setNetworkBootOpts(
	ctx context.Context,
	m *dhcpv4.DHCPv4,
	n *data.Netboot,
) dhcpv4.Modifier {
	...
	withNetboot := func(d *dhcpv4.DHCPv4) {
		...
		if val := m.Options.Get(dhcpv4.OptionClassIdentifier); val != nil {
			if strings.HasPrefix(string(val), dhcp.HTTPClient.String()) {
				d.UpdateOption(
					dhcpv4.OptGeneric(dhcpv4.OptionClassIdentifier, []byte(dhcp.HTTPClient)),
				)
			}
		}
...
func (h *Handler) bootfileAndNextServer(
	ctx context.Context,
	pkt *dhcpv4.DHCPv4,
	customUC dhcp.UserClass,
	tftp netip.AddrPort,
	ipxe, iscript *url.URL,
) (string, net.IP) {
